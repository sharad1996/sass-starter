...
/**
 * When `NEXT_PUBLIC_API_URL` is unset or empty, the browser calls same-origin `/api/v1/...`
 * (see `next.config.ts` rewrites for `/api/v1/*`) so httpOnly refresh cookies match the page host
 * (fixes signup when the UI is `127.0.0.1` but the API was `localhost`, etc.).
 */
/** Builds the browser fetch URL for an API path (used by `apiRequest` and unit tests). */
...
async function parseJson<T>(res: Response): Promise<T> {
  ...
  try {
    ...
    throw new ApiError("Invalid response from server", res.status || 500);
  }
}
